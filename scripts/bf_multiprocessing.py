...
def try_decrypt(passphrase, password_found_event, shared_result):
    ...
    try:
        ...
        print(f"Proceso {Process().name} probando contraseña: {passphrase}")
...
def brute_force_process_worker(all_passwords, password_found_event, shared_result):
...
def brute_force_attack():
    ...
    for length in range(1, 4):  # Cambia '4' por un número mayor si necesitas probar más longitud de contraseñas
        ...
        for i in range(NUM_PROCESSES):
            ...
            process = Process(target=brute_force_process_worker, args=(process_passwords, password_found_event, shared_result))
...
# Iniciar el ataque de fuerza bruta multiproceso
brute_force_attack()
